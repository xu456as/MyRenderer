...
using namespace BasicMath;
	...
	*/
	...
	float BasicMath::operator* (const Vector4& x, const Vector4& y) {
		float sum = 0.0f;
		for (int i = 0; i < 3; ++i) {
			sum += x[i] * y[i];
		}
	...
	Vector4 BasicMath::operator+ (const Vector4& x, const Vector4& y) {
		return Vector4(x[0] + y[0], x[1] + y[1], x[2] + y[2], x[3] + y[3]);
	}

	Vector4 BasicMath::operator* (const Vector4& x, const float& num) {
	...
	*/
	...
	Matrix4x4 BasicMath::operator* (const Matrix4x4& x, const float& num) {
		Matrix4x4 retMat;
		for (int i = 0; i < 4; ++i) {
			for (int j = 0; j < 4; ++j) {
	...
	int BasicMath::gcd(int x, int y) {
		return y == 0 ? x : (BasicMath::gcd(y, x%y));
	}

	bool BasicMath::EqualF(float f1, float f2) {
